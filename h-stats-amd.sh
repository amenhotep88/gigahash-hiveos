...
CUSTOM_DIR="/hive/miners/custom/gigahash-amd"
[[ -f "$CUSTOM_DIR/h-manifest.conf" ]] && . "$CUSTOM_DIR/h-manifest.conf"
LOG="${CUSTOM_LOG_BASENAME:-/var/log/miner/gigahash-amd/gigahash-amd}.log"
NATIVE_STATS="${CUSTOM_LOG_BASENAME:-/var/log/miner/gigahash-amd/gigahash-amd}.json"
khs=0
stats='null'
...
if [[ -s "$NATIVE_STATS" ]] && command -v jq >/dev/null 2>&1 && jq -e . "$NATIVE_STATS" >/dev/null 2>&1; then
  ...
  miner_ver="$(jq -r '.miner_version // "2.5"' "$NATIVE_STATS")"
  ...
  if [[ "$last_total" =~ Total[[:space:]]+([0-9.]+)[[:space:]]+(p/s|kp/s|Mp/s) ]]; then
    total_value="${BASH_REMATCH[1]}"; total_unit="${BASH_REMATCH[2]}"
    khs="$(awk -v v="$total_value" -v u="$total_unit" 'BEGIN { if (u=="p/s") v=v/1000; else if (u=="Mp/s") v=v*1000; printf "%.3f", v }')"
  fi
  parsed="$(tail -n 220 "$LOG" | awk -F'|' '
    function trim(s) { gsub(/^[ \t]+|[ \t]+$/, "", s); return s }
    $2 ~ /^[ \t]*[0-9]+[ \t]*$/ && $3 ~ /p\/s/ {
      id=trim($2)+0; split(trim($3),a,/[ \t]+/); v=a[1]+0; u=a[2]
      if (u=="p/s") v=v/1000; else if (u=="Mp/s") v=v*1000
      rate[id]=v; t=trim($5); sub(/[ \t]*C.*/,"",t); temp[id]=t+0
      f=trim($6); sub(/%.*/,"",f); fan[id]=f+0; if (id>max) max=id
    }
  ...
  [[ "$uptime" =~ ^[0-9]+$ ]] || uptime=0
  stats="$(printf '{"hs":%s,"hs_units":"khs","temp":%s,"fan":%s,"uptime":%s,"ver":"2.5"}' "$hs_json" "$temp_json" "$fan_json" "$uptime")"
fi
