...
PACKAGE="$REPO_DIR/gigahash-amd-2.5.0.tar.gz"
STAGE="$(mktemp -d)"
...
[[ -s "$PACKAGE" ]] || fail 'AMD v2.5 HiveOS package was not built'
...
grep -qx 'CUSTOM_VERSION=2.5.0-amd1' "$ROOT/h-manifest.conf" || fail 'wrong AMD package version'
...
echo 'PASS: GigaHash v2.5 AMD HiveOS package behavior'
